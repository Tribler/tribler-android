...
ActivityInstrumentationTestCase2<MainActivity>{
	...
	private void checkSingleMessage(int messageResource, int messageVisibility, int progressBarvisibility) {
		assertEquals("message is not visible.", messageVisibility, message.getVisibility());
		assertEquals("Message text is incorrect.", getActivity().getResources().getText(messageResource), message.getText());
		assertEquals("ProgressBar visibility is not correct.", progressBarvisibility, progressBar.getVisibility());
	}
